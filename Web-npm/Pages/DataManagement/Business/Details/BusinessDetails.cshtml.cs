...
namespace Web.Pages.DataManagement.Business.Details
{
    ...
    public class BusinessDetailsModel : CustomPageModel
    {
        ...
        public async Task<IActionResult> OnGet(long id)
        {
            if (id > 0)
            {
                Core.AppContext.Current.Services.BusinessService.Request = new Core.Request.Business.BusinessRequest() { BusinessId = id };
                var entity = await Core.AppContext.Current.Services.BusinessService.GetById();
