...
namespace Web0.Controllers.User
{
    ...
    {
        private Service<BusinessRequest, BusinessModel> BusinessService => Services.BusinessService;
        [Route("lookup")]
        [HttpGet]
        ...
        {
            var model = await BusinessService.GetById();
            if (model == null || model.BusinessId < 1 || model.CreatedByUserId != CurrentUser.UserId) return BadRequest();
