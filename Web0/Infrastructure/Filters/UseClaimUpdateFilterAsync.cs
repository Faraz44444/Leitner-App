...
namespace Web0.Infrastructure.Filters
{
    public class UserClaimUpdateFilterAsync : IAsyncResourceFilter
    {
        public async Task OnResourceExecutionAsync(ResourceExecutingContext context, ResourceExecutionDelegate next)
        {
            ...
            {
                var securityService = Core.AppContext.Current.NewServices.SecurityService;

                if (await securityService.IsUserUpdated(currentUser.UserId, currentUser.LastUpdateAt))
                {
                    Core.AppContext.Current.NewServices.UserService.Request = new Core.Request.User.UserRequest() { UserId = currentUser.UserId };
                    var user = await Core.AppContext.Current.NewServices.UserService.GetFirstOrDefault();
